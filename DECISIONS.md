...
4. **Skills surfaced.** Loader now projects the `skills` LIST column; a top-25 bar queries it via a subquery `unnest(skills)` (verified working in DuckDB-WASM). A d3-cloud word cloud shipped alongside it but was cut on first user review (layout quality did not justify the non-vanilla exception); the bar chart is the skills surface. A page-scoped Role/title `Inputs.select` (top-25 titles, unfiltered options to avoid a reactive cycle) renders INSIDE the shared filter card via a new `extras` slot on `filterCard` — visually integrated, but its value is not URL-persisted and does not carry across pages.
...
7. **`work_arrangement` is a global filter.** Added to the shared filterCard on every data page (options: remote / hybrid / onsite / unknown, where unknown → `work_arrangement IS NULL`), composed in `whereClause` and URL-persisted as `arr` — selector carries across pages like the other global filters (ADR-024 semantics).
8. **Smoke coverage.** Dev-phase additions: per-page CSV-blob assertion (monkey-patched `URL.createObjectURL`) and a /geography map phase (path count, filled-path assertion, metric-switch re-render).
...
- The role filter's non-persistence is a deliberate trade-off; revisit only if cross-page title analysis becomes a real workflow. Its options list is deliberately unfiltered (top-25 overall) — deriving it from the other filters would create a cycle through the shared form.
- `feature/dashboard-next` is now fully superseded (its only unmerged value was dataTable.js) and can be deleted.
- **Postings `country` codes are UPPERCASE ISO2 (GB/ES)** — a lowercase assumption in the first choropleth cut made the fill join silently empty (map rendered outlines only). The join normalizes case and the smoke map phase now asserts filled paths.
- No custom-viz deviations from stock Plot remain (the d3-cloud experiment was reverted); [[feedback-observable-defaults]] holds unqualified.
