...
| 14 | **Ops hygiene bundle** — Adzuna attribution footer · gate calibration on 18 manifests · artifact correctness gate · PR-gate smoke for `site/**` · `raw_payload` drop | ★★★ | S each | Low | Attribution is 15 min and a ToS risk. `raw_payload` drop shrinks public asset + ToS surface. Gate calibration finally data-grounded. |
...
**Sprint A — legibility (~1 day):** #1 README hero → #2 widen countries, let Monday cron run → ~~#4 F2~~ (merged PR #49) → #14 attribution footer.
