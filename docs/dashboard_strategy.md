...
> **✅ Principle 8 reinstated 2026-05-19.** §2 principle 8 (SQL composition centralised in-browser via DuckDB-WASM) is the Framework-idiomatic pattern. Observable explicitly says: *"The most convenient way to use DuckDB in Observable is the built-in SQL code blocks and `sql` tagged template literal."* The earlier "supersede by build-time loaders + d3.rollup" framing was wrong-headed — DuckDB-WASM is a first-class Framework citizen and SQL flexibility is a design value (see `feedback_duckdb_first_class_in_framework` memory). Concrete pattern: front-matter `sql: { table: ./data/file.parquet }` registers the table, ```sql id=name``` fenced blocks return Arrow tables, `sql` template tag handles dynamic queries. The rebuild planned in §3.3 / §3.4 uses this pattern, NOT `db.query()` with manual `DuckDBClient.of()`. **Caveat to document, not engineer around:** [duckdb-wasm issue #1658](https://github.com/duckdb/duckdb-wasm/issues/1658) — Chrome/Edge on Windows mishandles cached parquet bytes on page nav, throwing `TProtocolException`; Firefox unaffected; user mitigation is hard-refresh / Firefox. **If/when the accumulated `latest-{preset_id}.parquet` reaches the 150-250 MB regime** (ADR-020), revisit the trade-off — DuckDB-WASM cold-load may become untenable at that scale, but until then it's the canonical pattern. Full re-evaluation: `docs/sessions/2026-05-19-dashboard-multi-page-shipped.md`.
