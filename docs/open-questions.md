...
Goal: prove the weekly GitHub Actions pipeline is *correct*, not just green. A 4-track review on 2026-07-21 (extraction code, docs-vs-reality, automation readiness, Adzuna upstream API/ToS) established: the automation is code-complete and has run green weekly since 2026-06-08; secrets/Pages/permissions are already configured in the repo. Working branch for this phase: `feat/pipeline-verification`.

**Done 2026-07-21, merged to `main`** (see Resolved): docs-vs-reality cleanup (PR #32, 12 files); pipeline hardening (PR #33, `f7f6e65`) — credential-leak fix in `adzuna.py` error wrapping + traceback-scrubbing filter on root handlers; retry restructured (was dead code — conversion to `SourceFetchError` inside the retried function hid httpx errors from tenacity) and restricted to 5xx/429/transport; `max_results` truncation warning; NaT `posted_at` quarantine; `accumulated_row_count` manifest annotation; `refresh.yml` failure-alert issue step. **First cron with the hardened code: Monday 2026-07-27 — verify it goes green and the manifest carries the new fields.**
