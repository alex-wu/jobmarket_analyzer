...
- **Dashboard surface for `work_arrangement`** — IN PROGRESS on branch `feat/dashboards-work-arrangement` (2026-06-04). Loader `site/src/data/postings.parquet.js` SELECT now includes `work_arrangement` (uncommitted working edit). Still TODO: a chart slot (likely `index.md` or a new "dynamics" page) + decide the local sample-data strategy — the committed `gh_databuild_samples` sample is still schema v2; currently swapped locally to the 2026-05-29 v3 artifact (backed up to `.v2bak`). Viz must handle the dominant NULL bucket honestly (~78% NULL on the /details/-ON run, more on truncated-only). See `docs/sessions/2026-06-04-schema-v3-merge-dashboard-bootstrap.md`.
