...
## Next phase — pipeline automation verification (branch `feat/pipeline-automation`, opened 2026-07-20)

Goal: prove the weekly GitHub Actions pipeline is *correct*, not just green. The refresh cron has been succeeding weekly and now publishes schema-v3 assets, but "success" is only workflow exit status.

- **Artifact correctness gate** — validate the published `latest-{preset_id}.parquet` + `manifest.json` after each cron: schema_version matches, row_count sane vs prior week (alert on >50% drop), pandera-validate the published parquet, work_arrangement/skills coverage within expected bands.
- **Failure loudness** — a red cron currently just sits in the Actions tab. Decide: GitHub issue auto-open, or commit-status badge in README, or notification.
- **Accumulation window audit** — `export_accumulated()` unions 180 days of dated releases; verify dated-release retention and that the union actually dedupes as intended on real multi-week data (first_seen_at/last_seen_at drift).
- **Pages rebuild coupling** — confirm the pages workflow rebuilds after each data refresh (not only on push-to-main) so the deployed dashboard doesn't serve stale data for a week.
- **PR-gate smoke** (carried over) — path-filtered smoke on PRs touching `site/**`; would have caught this session's choropleth-join bug pre-merge.
- Carried context: preset switcher still queued (loader hardcodes `data_analyst_eu`); ES keyword dictionary still thin (ES work_arrangement 88% NULL).
